...
// Import kicks off a BACKGROUND CSV import of order_items. It streams the upload
...
func (h *OrderItemHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsOrderItem(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "order_items", Status: "processing", Total: total}
	...
	go h.runImportOrderItem(job.ID, tmpPath)
...
// countCSVRowsOrderItem counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsOrderItem(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportOrderItem streams the temp CSV, creating order_items in batches and
...
func (h *OrderItemHandler) runImportOrderItem(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.OrderItem, len(batch))
		for i := range batch {
	...
	for {
		...
		item := models.OrderItem{}
		if v, ok := get(rec, "product"); ok && v != "" {
			var rel models.Product
			if err := h.DB.Where("name = ?", v).First(&rel).Error; err != nil {
				rel = models.Product{Name: v}
				h.DB.Create(&rel)
			}
			item.ProductID = rel.ID
		}
		if v, ok := get(rec, "product_name"); ok {
			item.ProductName = v
		}
		if v, ok := get(rec, "quantity"); ok {
			n, _ := strconv.Atoi(v)
			item.Quantity = n
		}
		if v, ok := get(rec, "unit_price"); ok {
			n, _ := strconv.ParseFloat(v, 64)
			item.UnitPrice = n
		}
		if v, ok := get(rec, "line_total"); ok {
			n, _ := strconv.ParseFloat(v, 64)
			item.LineTotal = n
		}
		if v, ok := get(rec, "order_id"); ok && v != "" {
			var rel models.Order
...
// Template returns a ready-to-fill CSV template (header row) for importing order_items.
...
func (h *OrderItemHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="order-items-template.csv"`)
	c.String(http.StatusOK, "product,product_name,quantity,unit_price,line_total,order_id\n")
}
