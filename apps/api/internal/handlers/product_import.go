...
// Import kicks off a BACKGROUND CSV import of products. It streams the upload
...
func (h *ProductHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsProduct(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	go h.runImportProduct(job.ID, tmpPath)
...
// countCSVRowsProduct counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsProduct(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportProduct streams the temp CSV, creating products in batches and
...
func (h *ProductHandler) runImportProduct(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.Product, len(batch))
		for i := range batch {
	...
	for {
		...
		if v, ok := get(rec, "sku"); ok {
			item.Sku = v
		}
		if v, ok := get(rec, "description"); ok {
			item.Description = v
		}
		if v, ok := get(rec, "price"); ok {
			n, _ := strconv.ParseFloat(v, 64)
			item.Price = n
		}
		if v, ok := get(rec, "compare_at_price"); ok {
			n, _ := strconv.ParseFloat(v, 64)
			item.CompareAtPrice = n
		}
		if v, ok := get(rec, "stock"); ok {
			n, _ := strconv.Atoi(v)
			item.Stock = n
		}
		...
		if v, ok := get(rec, "active"); ok {
			item.Active = v == "true" || v == "1" || v == "yes"
		}
...
// Template returns a ready-to-fill CSV template (header row) for importing products.
...
func (h *ProductHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="products-template.csv"`)
	c.String(http.StatusOK, "name,sku,description,price,compare_at_price,stock,category,active\n")
}
