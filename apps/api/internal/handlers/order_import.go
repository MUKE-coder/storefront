...
// Import kicks off a BACKGROUND CSV import of orders. It streams the upload
...
func (h *OrderHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsOrder(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "orders", Status: "processing", Total: total}
	...
	go h.runImportOrder(job.ID, tmpPath)
...
// countCSVRowsOrder counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsOrder(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportOrder streams the temp CSV, creating orders in batches and
...
func (h *OrderHandler) runImportOrder(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.Order, len(batch))
		for i := range batch {
	...
	for {
		...
		item := models.Order{}
		if v, ok := get(rec, "number"); ok {
			item.Number = v
		}
		if v, ok := get(rec, "customer_name"); ok {
			item.CustomerName = v
		}
		if v, ok := get(rec, "customer_email"); ok {
			item.CustomerEmail = v
		}
		if v, ok := get(rec, "shipping_address"); ok {
			item.ShippingAddress = v
		}
		if v, ok := get(rec, "phone"); ok {
			item.Phone = v
		}
		if v, ok := get(rec, "subtotal"); ok {
			n, _ := strconv.ParseFloat(v, 64)
			item.Subtotal = n
		}
		if v, ok := get(rec, "shipping"); ok {
			n, _ := strconv.ParseFloat(v, 64)
			item.Shipping = n
		}
		if v, ok := get(rec, "total"); ok {
			n, _ := strconv.ParseFloat(v, 64)
			item.Total = n
		}
		if v, ok := get(rec, "payment_intent"); ok {
			item.PaymentIntent = v
		}
...
// Template returns a ready-to-fill CSV template (header row) for importing orders.
...
func (h *OrderHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="orders-template.csv"`)
	c.String(http.StatusOK, "number,customer_name,customer_email,shipping_address,phone,subtotal,shipping,total,payment_intent,status\n")
}
