...
// Import kicks off a BACKGROUND CSV import of categories. It streams the upload
...
func (h *CategoryHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsCategory(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "categories", Status: "processing", Total: total}
	...
	go h.runImportCategory(job.ID, tmpPath)
...
// countCSVRowsCategory counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsCategory(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportCategory streams the temp CSV, creating categories in batches and
...
func (h *CategoryHandler) runImportCategory(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.Category, len(batch))
		for i := range batch {
...
// Template returns a ready-to-fill CSV template (header row) for importing categories.
...
func (h *CategoryHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="categories-template.csv"`)
	c.String(http.StatusOK, "name,description,featured,parent\n")
}
