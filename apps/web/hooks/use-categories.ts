...
export function useCategories({ page = 1, pageSize = 20, search = "", sortBy = "created_at", sortOrder = "desc" }: UseCategoriesParams = {}) {
  return useQuery<CategoriesResponse>({
    queryKey: ["categories", { page, pageSize, search, sortBy, sortOrder }],
    queryFn: async () => {
...
export function useUpdateCategory() {
  ...
  return useMutation({
    mutationFn: async ({ id, ...input }: { id: string } & Record<string, unknown>) => {
      const { data } = await apiClient.put(`/api/categories/${id}`, input);
      return data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["categories"] });
